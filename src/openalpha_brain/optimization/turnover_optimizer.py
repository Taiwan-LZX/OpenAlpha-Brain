...
class TurnoverOptimizer:
    ...
    def generate_low_turnover_variants(
        ...
        try:
            ...
            for _strategy_name, generator in strategy_generators:
                try:
        ...
        try:
            ...
            for i, v in enumerate(variants):
                variant_summaries.append(
                    f"  [{i}] {v.expression[:200]} (strategy={v.strategy}, ΔTO≈{v.expected_to_reduction:.0%}, risk={v.risk_level})",  # noqa: E501
                )
    ...
    def _generate_mean_variants(self, expr: str, turnover: float) -> list[Variant]:
        ...
        if not decay_matches:
            ...
            for match in decay_matches[:2]:
                ...
                windows_to_try = list({max(old_window, 10), max(old_window, 20), old_window + 10})
    ...
    def _generate_double_smoothing_variants(self, expr: str, turnover: float) -> list[Variant]:
        ...
        outer_windows = [10, 15, 20] if has_existing_decay else [5, 10, 15]
    ...
    def optimize(
        ...
        try:
            ...
            [v for v in variants if v.risk_level in ("low", "medium")]
