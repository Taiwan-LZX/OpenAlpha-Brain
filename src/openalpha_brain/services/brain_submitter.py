...
def _parse_expression_fingerprint(expression: str) -> dict:
    ...
    field_pattern = re.compile(
        r"\b(close|open|high|low|vwap|volume|adv\d+|returns|cap|sales|assets|equity|revenue|earnings|sharesout)\b",
        re.IGNORECASE,
    )
...
def _apply_diversity_injection(
    ...
    try:
        ...
        similarity_score = root_ratio * 0.35 + comp_ratio * 0.25 + avg_field_overlap * 0.20 + avg_op_overlap * 0.20
        result["similarity_score"] = round(similarity_score, 4)

        homogenized = root_ratio > 0.60 or comp_ratio > 0.60 or similarity_score > 0.55
        ...
        candidate_ops = [op for op, alts in _OPERATOR_SWAP_MAP.items() if op in current_ops_set]
        ...
        if not suggested_operators:
            fallback_ops = ["ts_regression", "ts_corr", "ts_av_diff", "ts_skewness", "signed_power"]
        ...
        candidate_fields = [f for f, alts in _FIELD_SWAP_MAP.items() if f.lower() in current_fields_set]
        ...
        if feature_map is not None:
            try:
                ...
                underused_features = [f"方向={d} (覆蓋率 {dir_cov[d]:.0%})" for d in underused_dirs[:3]]
                ...
                logger.debug(
                    "[DEFENSIVE_LOG] _apply_diversity_injection: feature_map 获取未探索方向失败，跳过该增强: %s", exc
                )
        ...
        if suggested_operators:
            ...
            prompt_parts.append(
                f"  [運算子替換] 當前使用 '{current_root}' 系列運算子。改用以下未被充分探索的運算子：{op_list}"
            )
            prompt_parts.append(
                f"  例如：rank(ts_delta(close, 5)) → zscore(ts_regression({suggested_fields[0] if suggested_fields else 'vwap'}, 10))"  # noqa: E501
            )

        if suggested_fields:
            fld_list = ", ".join(suggested_fields[:3])
            prompt_parts.append(
                f"  [欄位跳躍] 當前使用價格/成交量欄位 ({', '.join(list(current_fields_set)[:4]) if current_fields_set else 'N/A'})。"  # noqa: E501
                f"引入基本面/替代資料欄位：{fld_list}"
            )

        if underused_features:
            feat_list = "; ".join(underused_features[:3])
            prompt_parts.append(f"  [FeatureMap 探索獎勵] 以下特徵空間幾乎未被佔據：{feat_list}")

        if current_comp == "multiplicative":
            prompt_parts.append("  [拓撲變換] 當前為乘法組成 → 建議改為條件式 (trade_when) 或加法組成")
        elif current_comp == "single":
            prompt_parts.append("  [拓撲變換] 當前為單一訊號 → 建議改為多因子交互作用 (* 或 / 組合)")
...
def _apply_elite_crossover(
    ...
    try:
        ...
        for elite in sorted_elites:
            ...
            has_new_fundamental = bool(
                elite_fields - current_fields & {"cap", "earnings", "sales", "assets", "revenue"}
            )
        ...
        if elite_root != current_root and elite_root in _OPERATOR_SWAP_MAP.get(current_root, []):
            ...
            expected_improvements.append(f"運算子替換：'{current_root}'→'{elite_root}' 可能改變時序平滑特性")
        ...
        if new_fundamental_fields:
            ...
            replace_parts.append(f"引入精英 alpha 的基本面欄位：{fld_names}")
            expected_improvements.append("基本面欄位注入可能降低與價格因子的相關性")

        new_price_fields = elite_fields - current_fields & {"open", "high", "low", "vwap"}
        if new_price_fields and not new_fundamental_fields:
            fld_names = ", ".join(sorted(new_price_fields)[:3])
            replace_parts.append(f"替換為精英 alpha 使用的價格欄位：{fld_names}")
        ...
        for old_op, new_op in smooth_swap_pairs:
            if old_op in current_ops and new_op in elite_ops:
                replace_parts.append(
                    f"平滑方式替換：{old_op} → {new_op} (精英 alpha 使用此方式達到 Sharpe={best_elite['sharpe']:.2f})"
                )
                expected_improvements.append(f"{new_op} 可能有不同的延遲/衰減特性，影響 turnover 和 sharpe")
                break
...
def _log_brain_result(state, result, expression: str, attempt: int) -> None:
    ...
    if failures:
        for chk in failures:
            _log(
                state,
                "BRAIN_FAIL",
                f"BRAIN check FAILED: {chk['name']} (value={chk.get('value')}, limit={chk.get('limit')})",
                {"check": chk["name"], "value": chk.get("value"), "limit": chk.get("limit"), "attempt": attempt},
            )
    elif err_msg:
        tag = "BRAIN_ERROR" if result.status == BrainSimStatus.ERROR else "BRAIN_FAIL"
        for em in err_msg[:3]:
            _log(state, tag, f"BRAIN simulation {result.status.value}: {em[:120]}", {"attempt": attempt})
    ...
    _log(
        ...
        f"Returns={result.real_returns}%",
        {
            ...
            "fitness": result.real_fitness,
            "turnover": result.real_turnover,
            "returns": result.real_returns,
        },
    )
...
async def create_dedup_mutation_callback(expression: str, result: dict) -> dict | None:
    ...
    """
    dup_checks = (
        result.get("brain_checks", []) if isinstance(result, dict) else getattr(result, "brain_checks", []) or []
    )
    dup_details = [
    ...
    mutation_prompt = f"""You are a quantitative alpha factor engineer. The following alpha was flagged as DUPLICATE (self_correlation > 0.7) by the BRAIN platform.  # noqa: E501
    ...
    try:
        ...
        if parsed and isinstance(parsed, dict):
            ...
            if variant_expr and variant_expr != expression:
                ...
                return parsed
    except (TimeoutError, aiohttp.ClientError, ValueError, json.JSONDecodeError) as exc:  # noqa: SIM105
...
async def _submit_to_brain(alpha, session_id: str, cycle_num: int):
    ...
    with contextlib.suppress(OSError, ValueError, RuntimeError):
        await tel.record_enter(
            "BrainSubmitter", cycle_id=session_id, expr_id=hash(getattr(alpha, "alpha_id", "")) % 10000
        )
    ...
    if _brain_cb.is_open:
        logger.warning(
            ...
            session_id,
            cycle_num,
            _brain_cb._last_failure_reason[:100],
        )
    ...
    if not sim_payload.get("settings") or not sim_payload.get("regular"):
        logger.warning(
            "[%s] cycle=%d BRAIN_SKIP: no simulation_payload on alpha %s",
            session_id,
            cycle_num,
            getattr(alpha, "alpha_id", "?"),
        )
        return BrainSubmissionResult(
            status=BrainSimStatus.ERROR,
            real_sharpe=None,
            ...
            while _auth_attempts < _max_auth_retries:
                try:
                    ...
                    if _auth_attempts >= _max_auth_retries:
                        logger.error(
                            "[%s] cycle=%d BRAIN_AUTH_TIMEOUT after %d retries",
                            session_id,
                            cycle_num,
                            _max_auth_retries,
                        )
                    ...
                    logger.warning(
                        "[%s] cycle=%d BRAIN_AUTH_RETRY %d/%d in %.1fs",
                        session_id,
                        cycle_num,
                        _auth_attempts,
                        _max_auth_retries,
                    ...
                    _delay = _auth_base_delay * (2 ** (_auth_attempts - 1))
                    logger.warning(
                        "[%s] cycle=%d BRAIN_AUTH_RETRY %d/%d: %s",
                        session_id,
                        cycle_num,
                        _auth_attempts,
                        _max_auth_retries,
    ...
    *,
    ...
    if initial_result is None:
        if not expression and not initial_expression:
            logger.warning(
                "[%s] cycle=%d BRAIN_IMPROVE_SKIP: no initial_result and no expression", session_id, cycle_num
            )
            return None
        logger.info(
            "[%s] cycle=%d BRAIN_IMPROVE_DEGRADE: entering degradation mode with expression", session_id, cycle_num
        )
    ...
    try:
        ...
        logger.info(
            "[%s] cycle=%d [STABILITY] ✓ StabilityGuard initialized for improvement loop", session_id, cycle_num
        )
    except (ImportError, ValueError, OSError) as stab_init_exc:
        logger.warning(
            "[%s] cycle=%d [STABILITY] ⚠ Init failed (graceful degradation): %s", session_id, cycle_num, stab_init_exc
        )
    ...
    try:
        ...
        if failure_diagnosis and _failure_lib and settings.FAILURE_FIX_LIBRARY_ENABLED:
            try:
                ...
                logger.debug(
                    "[DEFENSIVE_LOG] brain_improvement: failure_lib add_failure 失败，不影响主流程: %s",
                    exc,
                    exc_info=True,
                )
    except (ValueError, TypeError, OSError, RuntimeError) as exc:
    ...
    try:
        ...
        logger.warning(
            "[%s] cycle=%d EXP_REPLAY_INIT_FAILED: %s — continuing without experience replay",
            session_id,
            cycle_num,
            exc,
        )
    ...
    if _EA_AVAILABLE:
        try:
            ...
            logger.info(
                "[%s] cycle=%d [EA] ✅ EASearchStrategy initialized (pop=%d, gen=%d)",
                session_id,
                cycle_num,
                _ea_config.population_size,
                _ea_config.max_generations,
            )
        except (ValueError, TypeError, OSError) as _ea_init_exc:
            logger.warning(
                "[%s] cycle=%d [EA] ⚠️ Init failed: %s — falling back to standard loop",
                session_id,
                cycle_num,
                _ea_init_exc,
            )
    ...
    if _TOT_AVAILABLE:
        try:
            ...
            logger.info(
                "[%s] cycle=%d [TOT] ✅ ToTSearchStrategy initialized (depth=%d, branch=%d)",
                session_id,
                cycle_num,
                _tot_config.max_depth,
                _tot_config.branch_factor,
            )
        ...
        try:
            if _stability_guard is not None and attempt > 0 and current_expr:
                try:
                    ...
                    logger.debug(
                        "[%s] cycle=%d [STABILITY] loop evaluation failed attempt=%d: %s",
                        session_id,
                        cycle_num,
                        attempt + 1,
                        stab_loop_exc,
            ...
            if attempt > 0:
                ...
                if _exp_replay is not None:
                    try:
                        ...
                        logger.debug(
                            "[DEFENSIVE_LOG] brain_improvement: exp_replay_query_failed attempt=%d error=%s — falling back to segment_mutator",  # noqa: E501
                            attempt + 1,
                            exc,
                        )
                    ...
                    try:
                        ...
                        if reflexion_result.passed_proxy:
                            ...
                            _log(
                                session_id,
                                "REFLEXION_REJECTED",
                                f"Attempt {attempt + 1}: factor rejected by proxy (score={reflexion_result.final_verdict.overall_score:.2f}), skipping submission",  # noqa: E501
                                {
                                    "cycle": cycle_num,
                                    "attempt": attempt + 1,
                                    "proxy_score": reflexion_result.final_verdict.overall_score,
                        ...
                        logger.warning(
                            "[%s] cycle=%d REFLEXION_FAILED attempt=%d: %s",
                            session_id,
                            cycle_num,
                            attempt + 1,
                            refl_exc,
                ...
                if _should_run_ea and _ea_strategy is not None:
                    ...
                    try:
                        ...
                        if _ea_best and _ea_best.fitness > _current_sharpe:
                            ...
                            if _ea_new_expr and _ea_new_expr != current_expr and len(_ea_new_expr) >= 5:
                                _log(
                                    session_id,
                                    "EA_SUCCESS",
                                    f"✅ EA found better solution: Sharpe {_current_sharpe:.3f} → {_ea_best.fitness:.3f}, "  # noqa: E501
                            ...
                            _log(
                                session_id,
                                "EA_NO_BETTER",
                                f"EA completed but no better solution found (best_fitness={_ea_best.fitness if _ea_best else 0:.3f}), continuing standard loop",  # noqa: E501
                                {"cycle": cycle_num, "attempt": attempt + 1},
                            )
                        ...
                        _log(
                            session_id,
                            "EA_TIMEOUT",
                            f"⏰ EA search timed out after {getattr(_ea_strategy, 'config', type('', (), {'timeout_seconds': 180})).timeout_seconds}s, falling back",  # noqa: E501
                            {"cycle": cycle_num, "attempt": attempt + 1},
                        )
                    except (
                        ValueError,
                ...
                if _tot_strategy is not None and attempt >= 2:
                    ...
                    if 0.9 <= _current_sharpe_tot < 1.25 and _current_fitness_tot < 1.0:
                        ...
                        _tot_trigger_reason = (
                            f"deep_search(sharpe={_current_sharpe_tot:.3f}, fitness={_current_fitness_tot:.3f})"
                        )
                ...
                if _should_run_tot and _tot_strategy is not None:
                    _log(
                        session_id,
                        "TOT_TRIGGERED",
                        f"🌳 Triggering ToT tree search: reason={_tot_trigger_reason}, seed_expr={current_expr[:60]}...",  # noqa: E501
                    ...
                    try:
                        ...
                        if _tot_best_expr and _tot_best_fitness > _current_fitness_tot:
                            if _tot_best_expr != current_expr and len(_tot_best_expr) >= 5:
                                _log(
                                    session_id,
                                    "TOT_SUCCESS",
                                    f"✅ ToT found better solution: Fitness {_current_fitness_tot:.3f} → {_tot_best_fitness:.3f}, "  # noqa: E501
                                    f"depth={_tot_result.total_depth_reached}, nodes={_tot_result.total_nodes_explored}, "  # noqa: E501
                            ...
                            _log(
                                session_id,
                                "TOT_NO_BETTER",
                                f"ToT completed but no better solution (best_fitness={_tot_best_fitness:.3f}), continuing",  # noqa: E501
                                {"cycle": cycle_num, "attempt": attempt + 1},
                            )
                        ...
                        _log(
                            session_id,
                            "TOT_TIMEOUT",
                            f"⏰ ToT search timed out after {getattr(_tot_strategy, 'config', type('', (), {'timeout_seconds': 240})).timeout_seconds}s, falling back",  # noqa: E501
                            {"cycle": cycle_num, "attempt": attempt + 1},
                        )
                    except (
                        ValueError,
    ...
    if _exp_replay is not None and _current_exp_card_id:
        try:
            final_status = (
                "PASS"
                if getattr(best_result, "status", None) == BrainSimStatus.PASS
                else (
                    "IMPROVED"
                    if (getattr(best_result, "real_sharpe", 0) or 0) > (getattr(initial_result, "real_sharpe", 0) or 0)
                    else "FAIL"
                )
            )
            final_metrics_after = {
            ...
            logger.debug(
                "[DEFENSIVE_LOG] brain_improvement: exp_replay_record_failed error=%s — outcome not recorded", exc
            )
    ...
    if _stability_guard is not None:
        try:
            ...
            _log(
                session_id,
                "STABILITY_REWARD_ADJUSTMENT",
                f"Final: base_reward={base_reward:.3f} → adjusted_reward={adjusted_reward:.3f} "
                f"(Δ={adjusted_reward - base_reward:+.3f})",
                {
                    "cycle": cycle_num,
                    "stability_score": _stability_guard.get_summary().get("current_stability_score", "N/A"),
                },
            )
            ...
            if hasattr(_param_optimizer, "_scheduler") and _param_optimizer._scheduler is not None:
                try:
                    ...
                    _log(
                        session_id,
                        "STABILITY_MAB_UPDATE",
                        f"MAB reward updated with stability adjustment | direction={direction} reward={adjusted_reward:.3f}",  # noqa: E501
                        {"cycle": cycle_num},
                    )
            ...
            logger.debug(
                "[%s] cycle=%d [STABILITY] final reward adjustment failed: %s", session_id, cycle_num, stab_final_exc
            )

    return best_result


async def _run_param_optimization(expression: str, session_id: str, cycle_num: int, **kwargs) -> dict | None:
    ...
    try:
        ...
        if _param_optimizer is not None:
            optimized = await _param_optimizer.optimize(expression=expression, session_id=session_id, **kwargs)
            if optimized:
                _log(
                    session_id,
                    "PARAM_OPT_COMPLETE",
        ...
        _log(session_id, "PARAM_OPT_SKIPPED", "Param optimizer not available or returned None", {"cycle": cycle_num})
        return None
...
class ProxyEvaluator:
    ...
    def evaluate(self, expr: str, context: dict = None) -> ProxyVerdict:
        with Timer("proxy_evaluate"):
            ...
            logger.info(
                "[DEFENSIVE_LOG] proxy_evaluator: expr=%.60s... score=%.3f submit=%s reason=%s",
                expr[:60],
                weighted_score,
                should_sub,
                decision_reason[:80],
    ...
    def _eval_historical_similarity(self, expr: str) -> tuple[float, str]:
        ...
        similar_count = 0
        for hist_expr, _hist_time in self._recent_submissions[-20:]:
            try:
...
class ReflexionEngine:
    def __init__(self, llm_generate_fn=None, proxy_evaluator: ProxyEvaluator = None, failure_fix_library=None):
    ...
    async def reflect_and_improve(
        self, original_expr: str, mutated_expr: str, failure: FailureDiagnosis, max_reflections: int = 2
    ) -> ReflexionResult:
        with Timer("reflexion_reflect_and_improve"):
            ...
            for round_i in range(max_reflections):
                ...
                if verdict.overall_score >= 0.70:
                    logger.info(
                        "[REFLEXION] Round %d: proxy passed (score=%.3f≥0.70), expr=%.60s...",
                        round_i + 1,
                ...
                logger.info(
                    "[REFLEXION] Round %d: proxy score=%.3f<0.70, triggering LLM reflection",
                    round_i + 1,
                    verdict.overall_score,
                )
                ...
                if not reflection.suggested_fix or reflection.confidence < 0.3:
                    logger.info(
                        "[REFLEXION] Round %d: LLM无有效建议或置信度过低(conf=%.2f)，停止反思",
                        round_i + 1,
                ...
                if current_expr != mutated_expr:
                    ...
                    if not integrity.get("valid", True):
                        logger.warning("[REFLEXION] Round %d: 反思后完整性验证失败，回滚到上一版本", round_i + 1)
                        current_expr = mutated_expr
                        break
            ...
            logger.info(
                "[REFLEXION] Completed: %d rounds, final_score=%.3f, passed=%s",
                len(reflection_log),
    ...
    async def _llm_reflect(
        self, expr: str, verdict: ProxyVerdict, failure: FailureDiagnosis, previous_reflections: list
    ) -> Reflection:
...
{{
  ...
  "confidence": 0.0-1.0
            ...
            try:
                ...
                logger.error(
                    "[DEFENSIVE_LOG] reflexion_engine: llm_reflect_parse_failed error=%s — returning fallback reflection",  # noqa: E501
                    exc,
                )
                return Reflection(
                    round_num=len(previous_reflections) + 1,
    ...
    def _apply_reflection_suggestion(self, original_expr: str, suggestion: str, target_block: str = "A") -> str:
        with Timer("reflexion_apply_suggestion"):
            ...
            if integrity.get("valid", False):
                log_call("reflexion_applied", extra={"old": original_expr[:50], "new": new_expr[:50]})
                return new_expr
            else:
                logger.warning("[REFLEXION] 应用建议后完整性验证失败: %s", integrity.get("warnings", []))
                return original_expr
...
class FailureClassifier:
    def __init__(self):
        self._rules = [
            ...
            (
                lambda m: m.get("sharpe", 0) < 0.3 and m.get("turnover", 0) < 0.25,
                FailureType.LOW_SHARPE,
                0.85,
                "mutate_A",
            ),
    ...
    def adjust_block_c(self, block_c: str, failure: FailureDiagnosis) -> str:
        with Timer("adjust_block_c"):
            ...
            if failure.failure_type in (FailureType.HIGH_TURNOVER, FailureType.DECAY_FAST):
                new_window = min(60, current_window * 2) if current_window < 20 else min(60, current_window + 10)
            else:
            ...
            log_call(
                "block_c_adjusted",
                extra={"old_window": current_window, "new_window": new_window, "reason": failure.failure_type.value},
            )
            return new_block_c
    ...
    def improve(self, expression: str, metrics: dict, error_msg: str = "") -> tuple:
        with Timer("segment_locked_improve"):
            ...
            log_call(
                "failure_classified",
                extra={
                    "type": diagnosis.failure_type.value,
                    "action": diagnosis.suggested_action,
                    "confidence": diagnosis.confidence,
                },
            )
...
class SignalQualityPreFilter:
    ...
    _OPERATOR_BLACKLIST_PATTERNS = [
        ...
        (r"^\s*-?\s*ts_delta\s*\(\s*volume\s*,\s*\d+\s*\)\s*$", "ts_delta(volume) 单独使用 - 过度拥挤信号"),
        (r"rank\s*\(\s*ts_mean\s*\([^)]+\)\s*\)", "rank(ts_mean(x)) 结构过于简单 - 仅2个算子"),
        (
        ...
        (r"^(?!.*(?:ts_decay_linear|ts_av_diff)).*$", "缺少 ts_decay_linear/ts_av_diff 的纯动量表达式 - 换手控制不足"),
    ]
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.info(
                "[DEFENSIVE_LOG] SignalQualityPreFilter: OperatorRegistry initialized with %d operators",
                len(self._op_reg.get_all_operator_names()),
            )
    ...
    def prefilter(self, expr: str, context: dict = None) -> PreFilterResult:
        ...
        if not layer3_result.passed:
            logger.info(
                "[DEFENSIVE_LOG] SignalQualityPreFilter [Layer3]: WQ格式预校验未通过 - %s", layer3_result.reason
            )
            self._record_failed_topology(expr)
        ...
        if not layer4_result.passed:
            logger.info(
                "[DEFENSIVE_LOG] SignalQualityPreFilter [Layer4]: 字段多样性检查未通过 - %s", layer4_result.reason
            )
            self._record_failed_topology(expr)
        ...
        if not layer5_result.passed:
            logger.info(
                "[DEFENSIVE_LOG] SignalQualityPreFilter [Layer5]: 结构指纹去重未通过 - %s", layer5_result.reason
            )
        ...
        final_confidence = min(
            layer2_result.confidence_score,
            layer3_result.confidence_score,
            layer4_result.confidence_score,
            layer5_result.confidence_score,
        )
        logger.debug(
            "[DEFENSIVE_LOG] SignalQualityPreFilter: 表达式通过所有预筛选检查 (confidence=%.2f)", final_confidence
        )
        return PreFilterResult(
            passed=True,
            reason="通过所有增强预筛选",
    ...
    def _check_complexity(self, expr: str) -> PreFilterResult:
        if self._op_reg is not None:
            ...
            logger.debug(
                "[DEFENSIVE_LOG] SignalQualityPreFilter: Using OperatorRegistry.count_operators() = %d", operator_count
            )
        else:
        ...
        confidence = 0.88 if 5 <= operator_count <= 12 else 0.78
    ...
    def _check_field_diversity(self, expr: str) -> PreFilterResult:
        ...
        fields_found = {f.lower() for f in field_pattern.findall(expr)}
        ...
        has_fundamental = bool(
            fields_found & {"cap", "earnings", "sales", "assets", "revenue", "sharesout", "debt", "enterprise_value"}
        )
        if has_fundamental:
            return PreFilterResult(
                passed=True,
    ...
    def _check_operator_blacklist(self, expr: str) -> PreFilterResult:
        if self._op_reg is not None:
            ...
            logger.debug(
                "[DEFENSIVE_LOG] SignalQualityPreFilter: Using OperatorRegistry forbidden patterns (count=%d)",
                len(patterns),
            )
    ...
    def _calculate_topology_similarity(topo1: str, topo2: str) -> float:
        ...
        if max_len == 0:
            return 1.0
        matches = sum(1 for a, b in zip(topo1, topo2, strict=False) if a == b)
    ...
    def _check_historical_similarity(self, expr: str) -> PreFilterResult:
        ...
        if similar_count > 0 or max_similarity > 0.90:
            return PreFilterResult(
                passed=False,
                reason=f"与近期提交高度相似（最大相似度={max_similarity:.1%}, 重复数={similar_count}）",
                confidence_score=0.87,
            )
