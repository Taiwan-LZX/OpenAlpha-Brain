    ...
    def detect_field_overfit(self) -> list[dict[str, Any]]:
        ...
        for field_id, count in counts.items():
            if count >= OVERUSE_THRESHOLD:
                ...
                warnings.append({
                    ...
                    "ratio": round(ratio, 3),
                    "message": f"Field '{field_id}' used {count}/{OVERUSE_WINDOW} times recently ({ratio:.1%}) — overfitting risk",
                })
        ...
        if len(datasets_in_window) < FIELD_DIVERSITY_MIN_DATASETS:
            warnings.append({
                ...
                "message": f"Only {len(datasets_in_window)} dataset(s) in recent {OVERUSE_WINDOW} fields — diversity too low",
            })
        return warnings
