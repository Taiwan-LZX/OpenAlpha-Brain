    ...
    for fail in syntax_result.failures:
        if "INVALID BRAIN variables" in fail:
            ...
            for bv in bad_vars:
                ...
                if bv:
                    _record_hallucination(
                        state, bv, "INVALID_VAR",
                        f"Variable '{bv}' is not in BRAIN data fields", "validator")
            ...
            for bo in bad_ops:
                ...
                if bo:
                    _record_hallucination(
                        state, bo, "UNKNOWN_OPERATOR",
                        f"Operator '{bo}' is not in BRAIN operator schema", "validator")
...
def _extract_brain_hallucinations(state, error_messages: list[str]) -> None:
    ...
    for em in error_messages[:3]:
        unknown_vars = re.findall(
            r"unknown variable\s+[\"']?([a-zA-Z_][a-zA-Z0-9_]*)[\"']?",
            em, re.IGNORECASE)
        for uv in unknown_vars:
            _record_hallucination(
                state, uv, "BRAIN_UNKNOWN_VAR",
                f"BRAIN rejected variable '{uv}': {em[:80]}", "brain")
...
def _is_fastexpr_line(line: str) -> bool:
    ...
    if re.match(r'^(\*\*|##|>|-|\d+\.)\s*', line):
        return False
    nl_start_words = (
        "Rationale", "Hypothesis", "Strategy", "Insight", "Note",
        "Idea", "Inefficiency", "This", "The", "When", "If", "For",
    )
...
def _apply_economic_rationale_verification(
    ...
    if not verification["valid"]:
        logger.warning(
            ...
            session_id, global_cycle, verification.get("reason", ""),
        )
...
def _extract_expression_from_llm(raw: str) -> str | None:
    ...
    for line in raw.splitlines():
        ...
        if not _is_fastexpr_line(line):
            continue
        if len(line) > 20:
            if any(op in line for op in ("ts_", "rank(", "group_neutralize(",
                                          "ts_decay_linear(", "signed_power(")):
    ...
    if stripped and "(" in stripped and ")" in stripped:
        _BARE_EXPR_OPS = (
            "rank(", "ts_delta(", "ts_mean(", "ts_std_dev(", "ts_zscore(",
            "ts_decay_linear(", "ts_rank(", "ts_delay(", "ts_sum(", "ts_corr(",
            "group_neutralize(", "group_rank(", "group_zscore(",
            "signed_power(", "zscore(", "normalize(", "winsorize(",
            "ts_av_diff(", "ts_quantile(", "quantile(",
        )
...
async def _handle_reject(state, session_id: str, global_cycle: int,
                         fingerprint_dict: dict, ast_topology, reason: str) -> None:
    """[Brief description of function purpose.]
...
async def _handle_iterate(state, session_id: str, global_cycle: int,
                          parsed: dict, expression: str, fingerprint_dict: dict,
                          ast_topology, all_failures: list, decision: str) -> None:
    """[Brief description of function purpose.]
...
def _build_alpha(state, parsed: dict, expression: str, fingerprint_dict: dict,
                 family: str, ast_topology, ast_collision, simulation_payload,
                 metrics_result, global_cycle: int, decision: str,
                 economic_rationale: str | None = None,
                 rationale_verification: dict | None = None) -> AlphaResult:
    """[Brief description of function purpose.]
