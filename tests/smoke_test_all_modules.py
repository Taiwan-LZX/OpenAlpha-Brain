...
def test_expression_validator():
    ...
    expr = "group_neutralize(  rank(ts_decay_linear(ts_delta(signed_power(-close / debt, 2), 5), 10)),  industry)"
...
def test_mutation_engine():
    """MutationEngine 基本诊断"""
    from openalpha_brain.evolution.mutation_engine import BrainAwareMutationEngine, MutationStrategy

    e = BrainAwareMutationEngine()
    d = e.diagnose("rank(ts_delta(close, 20))", {"sharpe": -0.5}, [])
    assert isinstance(d.strategy, MutationStrategy), f"Expected MutationStrategy, got {type(d.strategy)}"
