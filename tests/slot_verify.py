...
TEST_FACTORS = [
    {"name": "Mom_CloseOpen_10d",   "expr": "ts_decay_linear(group_neutralize(ts_rank(close - open, 10), industry), 20)",     "s": "momentum"},
    {"name": "Rev_VolumeDelta_5d",  "expr": "ts_decay_linear(group_neutralize(rank(ts_delta(volume / adv20, 5)), sector), 15)",     "s": "reversal"},
    {"name": "Vol_Range_Std_15d",   "expr": "ts_decay_linear(group_neutralize(ts_std_dev(high - low, 15), subindustry), 12)",      "s": "volatility"},
    {"name": "Trend_Accel_12d",     "expr": "ts_decay_linear(group_neutralize(ts_delta(ts_mean(close,5)-ts_mean(close,20),3), subindustry), 16)", "s": "trend"},
    {"name": "Liq_Spread_Corr_8d",  "expr": "ts_decay_linear(group_neutralize(ts_corr(volume, close-open, 8), industry), 18)",      "s": "liquidity"},
    {"name": "Price_ZScore_20d",    "expr": "ts_decay_linear(group_neutralize(ts_zscore(close, 20), sector), 10)",                "s": "mean_rev"},
    {"name": "Comp_MomRev_Hybrid",  "expr": "ts_decay_linear(group_neutralize(rank(ts_rank(close,5)) - rank(ts_rank(close,20)), sector), 18)", "s": "composite"},
    {"name": "Micro_OI_Change_6d",  "expr": "ts_decay_linear(group_neutralize(ts_av_diff(open_interest, 6), industry), 14)",         "s": "micro"},
    {"name": "Skew_Returns_20d",   "expr": "ts_decay_linear(group_neutralize(ts_skewness(close / delay(close,1) - 1, 20), sector), 10)", "s": "skewness"},
    {"name": "AccVol_Divergence",   "expr": "ts_decay_linear(group_neutralize(ts_corr(abs(return_0), volume, 10) - ts_corr(return_0, volume, 10), industry), 15)", "s": "anomaly"},
]
...
async def on_complete(slot, result):
    ...
    logger.info("  %s [Slot %d] %s Sharpe=%s TO=%s (%.0fs, %d polls)",
                icon, slot.slot_id, slot.task_name[:24], sp, to,
                slot.elapsed_sec, slot.poll_count)
...
async def status_reporter(manager, interval=15):
    ...
    while True:
        ...
        logger.info("--- STATUS: submitted=%d completed=%d passed=%d | slots_busy=%d/%d | queue=%d | best=%.3f ---",
                    m.total_submitted, m.total_completed, m.total_passed,
                    busy, len(slots), m.queue_depth, m.best_sharpe)
...
async def main():
    ...
    while elapsed < timeout_sec:
        ...
        if elapsed % 30 == 0:
            logger.info("  ... waiting: %d/%d done, queue=%d, best_sharpe=%.3f",
                        m.total_completed, len(TEST_FACTORS),
                        m.queue_depth, m.best_sharpe)
...
def print_final_report(results, total_time, manager):
    ...
    logger.info("Total Time: %.0fs | Throughput: %.1f alphas/min",
                total_time, total / max(total_time / 60, 0.01))
    logger.info("Submitted: %d | Completed: %d | Passed(Sharpe>=1.25): %d",
                m.total_submitted, m.total_completed, m.total_passed)
    logger.info("")
    ...
    logger.info("  %-3s %-26s %-10s %8s %7s %6s %6s  %s",
                "#", "Name", "Strategy", "Sharpe", "Fitness", "TO%", "Time", "Status/Fails")
    logger.info("  " + "-" * 95)
    ...
    for i, r in enumerate(sorted(results, key=lambda x: x.get("sharpe") or -999, reverse=True)):
        ...
        logger.info("  %-3d %-26s %-10s %8s %7s %6s %6s  %s %s",
                    i+1, r['name'][:24], r['strategy'][:8], sp, ft, to, tm, st, fl)
