...
try:
    from alpha_agent.datasets_loader import DatasetsLoader
    ...
    from alpha_agent.repair_engine import RepairEngine, ConvergenceGovernance
    from alpha_agent.sao import SignalAlphaObject
    _GRID_DEPS_AVAILABLE = True
except ImportError:
    _GRID_DEPS_AVAILABLE = False
    DatasetsLoader = None  # type: ignore[assignment,misc]
    ExplorationGrid = None  # type: ignore[assignment,misc]
    DatasetCategory = None  # type: ignore[assignment,misc]
    ExpressionGenerator = None  # type: ignore[assignment,misc]
    JaccardDiversityGate = None  # type: ignore[assignment,misc]
    LLMClient = None  # type: ignore[assignment,misc]
    DeterministicValidator = None  # type: ignore[assignment,misc]
    RAGSpecEncoder = None  # type: ignore[assignment,misc]
    RepairEngine = None  # type: ignore[assignment,misc]
    ConvergenceGovernance = None  # type: ignore[assignment,misc]
    SignalAlphaObject = None  # type: ignore[assignment,misc]
...
class ResearchToolbox:
    def __init__(self, runtime: AgentRuntimeConfig) -> None:
        ...
        self._rag_encoder: Optional[Any] = None
        self._deterministic_validator: Optional[Any] = None
        self._expression_generator: Optional[Any] = None
        self._repair_engine: Optional[Any] = None
        self._field_metadata: Optional[Dict[str, Any]] = None
    ...
    # ── Grid pipeline (feature-gated) ──────────────────────────────

    def ensure_grid(self) -> Any:  # type: ignore[return]
        if not self.agent_cfg.grid.enabled or not _GRID_DEPS_AVAILABLE:
            return None
        if self._grid is None:
            metadata = self._ensure_metadata()
            self._grid = ExplorationGrid(field_metadata=metadata)  # type: ignore[union-attr]
            if self.agent_cfg.grid.grid_populate_from_library:
                self._grid.populate_from_library(self.library)  # type: ignore[union-attr]
        return self._grid

    def _ensure_metadata(self) -> Optional[Dict[str, Any]]:
        if not _GRID_DEPS_AVAILABLE:
            return None
        if self._field_metadata is not None:
            return self._field_metadata
        datasets_dir = self.agent_cfg.datasets_dir
        try:
            loader = DatasetsLoader(datasets_dir)  # type: ignore[union-attr]
            loader.load()
            self._field_metadata = loader.all_metadata()
            logger.info("Loaded %d field metadata entries from %s", len(self._field_metadata), datasets_dir)
        except Exception as exc:
            logger.warning("Failed to load dataset metadata from %s: %s", datasets_dir, exc)
            self._field_metadata = {}
        return self._field_metadata

    def ensure_rag_encoder(self) -> Any:  # type: ignore[return]
        if not self.agent_cfg.rag.enabled or not _GRID_DEPS_AVAILABLE:
            return None
        if self._rag_encoder is None:
            meta = self._ensure_metadata()
            if meta:
                self._rag_encoder = RAGSpecEncoder.from_csv_metadata(meta)  # type: ignore[union-attr]
            else:
                self._rag_encoder = RAGSpecEncoder.from_fields_summary(  # type: ignore[union-attr]
                    self.agent_cfg.fields_summary
                )
        return self._rag_encoder

    def ensure_validator(self) -> Any:  # type: ignore[return]
        if not _GRID_DEPS_AVAILABLE:
            return None
        if self._deterministic_validator is None:
            meta = self._ensure_metadata()
            if meta:
                self._deterministic_validator = DeterministicValidator.from_csv_metadata(meta)  # type: ignore[union-attr]
            else:
                self._deterministic_validator = DeterministicValidator.from_fields_summary(  # type: ignore[union-attr]
                    self.agent_cfg.fields_summary
                )
        return self._deterministic_validator

    def ensure_expression_generator(self) -> Any:  # type: ignore[return]
        if not _GRID_DEPS_AVAILABLE:
            return None
        if self._expression_generator is None:
            diversity_threshold = self.agent_cfg.grid.grid_diversity_threshold
            gate = JaccardDiversityGate(threshold=diversity_threshold)  # type: ignore[union-attr]
            llm_client: Optional[Any] = None
            llm_gen_config = self.agent_cfg.llm_gen
            if llm_gen_config.enabled:
                llm_client = LLMClient(self.runtime.model)  # type: ignore[union-attr]
            self._expression_generator = ExpressionGenerator(  # type: ignore[union-attr]
                validator=self.ensure_validator(),
                rag_encoder=self.ensure_rag_encoder(),
                diversity_gate=gate,
    ...
    def ensure_repair_engine(self) -> Any:  # type: ignore[return]
        if not _GRID_DEPS_AVAILABLE:
            return None
        if self._repair_engine is None:
            gov = ConvergenceGovernance(  # type: ignore[union-attr]
                max_iterations=self.agent_cfg.grid.grid_max_repair_iterations,
                sharpe_threshold=self.agent_cfg.grid.grid_sharpe_threshold,
                delta_threshold=self.agent_cfg.grid.grid_delta_threshold,
            )
            llm_client: Optional[Any] = None
            if self.agent_cfg.llm_gen.enabled and self.agent_cfg.llm_gen.rewrite_enabled:
                llm_client = LLMClient(self.runtime.model)  # type: ignore[union-attr]
            self._repair_engine = RepairEngine(  # type: ignore[union-attr]
                governance=gov,
                llm_client=llm_client,
                rewrite_temperature=self.agent_cfg.llm_gen.rewrite_temperature,
            )
        return self._repair_engine

    def generate_grid_candidates(self) -> List[Any]:
        if not self.agent_cfg.grid.enabled or not _GRID_DEPS_AVAILABLE:
            return []
        grid = self.ensure_grid()
        if grid is None:
            return []
        generator = self.ensure_expression_generator()
        if generator is None:
            return []

        domain_dc = None
        if self.agent_cfg.grid.grid_domain_family:
            try:
                domain_dc = DatasetCategory(self.agent_cfg.grid.grid_domain_family)  # type: ignore[union-attr]
            except (ValueError, KeyError):
                pass

        cells = grid.select_cells(
            budget=self.agent_cfg.grid.grid_budget,
            domain_dc=domain_dc,
        )
        candidates: List[Any] = []
        frontier_exprs = [str(r.get("expression", "")) for r in self.results[-20:]]

        for cell in cells:
            generated = generator.generate_from_cell(cell, frontier_expressions=frontier_exprs)
            for gc in generated:
                if not gc.validation_passed:
                    continue
                thesis_str = "; ".join(t.short_str() for t in cell.thesis[:3]) if cell.thesis else ""
                sao = SignalAlphaObject(  # type: ignore[union-attr]
                    ...
                    family=gc.family,
                    diversity_score=gc.diversity_score,
                )
                candidates.append(sao)
        return candidates

    def repair_grid_candidate(self, sao: Any, failed_checks: Sequence[str]) -> Any:
        if not self.agent_cfg.grid.enabled or not _GRID_DEPS_AVAILABLE:
            return sao
        engine = self.ensure_repair_engine()
        if engine is None:
            return sao
        result = engine.execute(sao, failed_checks)
        sao.expression = result.expression
        sao.repair_iterations += 1
        sao.tried_strategies.append(result.strategy_name)
        return sao
...
class AlphaResearchAgent:
    ...
    def run(self) -> AgentRunResult:
        ...
        for iteration in range(1, self.runtime.agent.max_iterations + 1):
            ...
            if decision.action == "evaluate_grid":
                grid_records_count = self._execute_grid_action(
                    decision, iteration, current_stage, run_id,
                    remaining_budget, evaluated_total,
                )
                evaluated_total += grid_records_count
                evaluated_this_run += grid_records_count
                self.toolbox.write_state()
                self._emit_progress({
                    "type": "batch_completed", "run_id": run_id,
                    "iteration": iteration, "stage": current_stage,
                    "action": "evaluate_grid",
                    "details": {"executed_batch": grid_records_count},
                    "leaderboard": [
                        pipeline.compact_record(item)
    ...
    def _execute_grid_action(
        ...
        if not self.runtime.agent.grid.enabled or not _GRID_DEPS_AVAILABLE:
            self._append_event(
                iteration=iteration, stage=stage, action="evaluate_grid",
                rationale="Grid pipeline not enabled.", details={"result": "skipped"},
                hypothesis="", risk_note="",
            )
            return 0

        grid_saos = self.toolbox.generate_grid_candidates()
        if not grid_saos:
            self._append_event(
                iteration=iteration, stage=stage, action="evaluate_grid",
                rationale="No grid candidates generated.", details={"result": "empty"},
                hypothesis="", risk_note="",
            )
            return 0

        candidates = [sao.to_candidate() for sao in grid_saos[:max(1, decision.batch_size)]]
        self._emit_progress({
            "type": "batch_started", "run_id": run_id, "iteration": iteration,
            "stage": stage, "action": "evaluate_grid", "batch_size": len(candidates),
        ...
        records = self.toolbox.evaluate_candidates(
            candidates,
            progress_callback=lambda payload, it=iteration, st=stage: self._emit_progress({
                **payload, "run_id": run_id, "iteration": it, "stage": st,
                "action": "evaluate_grid", "budget": self.runtime.agent.budget,
            }),
        )
        repair_candidates = self._repair_grid_failures(grid_saos, records)
        total_records = len(records)

        if repair_candidates:
            repair_records = self.toolbox.evaluate_candidates(
                repair_candidates,
                progress_callback=lambda payload, it=iteration, st=stage: self._emit_progress({
                    **payload, "run_id": run_id, "iteration": it, "stage": st,
                    "action": "evaluate_grid_repair", "budget": self.runtime.agent.budget,
                }),
            )
            total_records += len(repair_records)
            repair_idx = 0
            for sao, rec in zip(grid_saos, records):
                if rec.get("status") != "ok":
                    failed = rec.get("failed_checks") or rec.get("failed_blocking_checks") or []
                    if not failed:
                        continue
                    if repair_idx >= len(repair_records):
                        break
                    repair_record = repair_records[repair_idx]
                    repair_idx += 1
                    cell_id = sao.source_cell_id
                    grid = self.toolbox.ensure_grid()
                    if grid:
                        if repair_record.get("status") == "ok":
                            sharpe = repair_record.get("sharpe") or repair_record.get("score")
                            grid.mark_result(cell_id, passed=True, sharpe=sharpe)
                        else:
                            grid.mark_result(cell_id, passed=False)
            self.notebook.observe_records(repair_records)

        self.notebook.observe_records(records)

        event_details = {
            "requested_batch": len(grid_saos),
            "executed_batch": len(records),
            "grid_cells": [sao.source_cell_id for sao in grid_saos],
        }
        self._append_event(
            iteration=iteration, stage=stage, action="evaluate_grid",
            rationale=decision.rationale, details=event_details,
            hypothesis=decision.hypothesis or "", risk_note=decision.risk_note or "",
        )
        return total_records

    def _repair_grid_failures(
        self, grid_saos: Sequence[Any], records: Sequence[Dict[str, Any]],
    ) -> List[Any]:
        if not self.runtime.agent.grid.enabled:
            return []
        repaired: List[Any] = []
        for sao, record in zip(grid_saos, records):
            if record.get("status") != "ok":
                failed = record.get("failed_checks") or record.get("failed_blocking_checks") or []
                if failed:
                    sao.failed_checks = list(failed)
                    self.toolbox.repair_grid_candidate(sao, failed)
                    repaired.append(sao.to_candidate())
                cell_id = sao.source_cell_id
                grid = self.toolbox.ensure_grid()
                if grid:
                    grid.mark_result(cell_id, passed=False)
                ...
                grid = self.toolbox.ensure_grid()
                if grid:
                    sharpe = record.get("sharpe") or record.get("score")
                    grid.mark_result(cell_id, passed=True, sharpe=sharpe)
        return repaired
    ...
    def _build_context(
        ...
        *,
        ...
        return {
            ...
            "grid_candidates_available": len(self.toolbox.generate_grid_candidates()) if self.runtime.agent.grid.enabled else 0,
        ...
        report = {
            ...
            "runtime": {
                ...
                "agent": {
                    ...
                    "allow_pending_checks": self.runtime.agent.allow_pending_checks,
                    "workdir": str(self.runtime.agent.workdir),
                    "grid_enabled": self.runtime.agent.grid.enabled,
                    "rag_enabled": self.runtime.agent.rag.enabled,
                    "llm_gen_enabled": self.runtime.agent.llm_gen.enabled,
                },
            },
            "summary": summary,
            "leaderboard": leaderboard,
