...
def sidebar_controls() -> tuple[AgentRuntimeConfig, bool, Dict[str, Any]]:
    with st.sidebar:
        ...
        st.markdown("### BrainAlpha Grid")
        grid_enabled = st.toggle("Enable exploration grid", value=False)
        grid_budget = st.number_input("Grid budget per iteration", min_value=1, max_value=50, value=5, step=1)
        grid_domain_family = st.text_input("Grid domain family (optional)", value="", placeholder="e.g. value, momentum")
        grid_diversity_threshold = st.slider("Grid diversity threshold", min_value=0.0, max_value=1.0, value=0.15, step=0.01)
        grid_max_repair = st.number_input("Grid max repair iterations", min_value=1, max_value=20, value=5, step=1)
        rag_enabled = st.toggle("Enable RAG field encoder", value=False, help="TF-IDF field recommendation")
        llm_gen_enabled = st.toggle("Enable LLM expression generation", value=False)
    ...
    runtime = AgentRuntimeConfig(
        ...
        agent=AgentConfig(
            ...
            grid=GridConfig(
                enabled=bool(grid_enabled),
                grid_budget=int(grid_budget),
                grid_domain_family=grid_domain_family.strip(),
                grid_diversity_threshold=float(grid_diversity_threshold),
                grid_max_repair_iterations=int(grid_max_repair),
            ),
            rag=RAGConfig(enabled=bool(rag_enabled)),
            llm_gen=LLMGenConfig(enabled=bool(llm_gen_enabled)),
        ),
    )
    preview = {
        ...
        "grid_enabled": runtime.agent.grid.enabled,
        "rag_enabled": runtime.agent.rag.enabled,
        "llm_gen_enabled": runtime.agent.llm_gen.enabled,
    }
    if run_clicked:
